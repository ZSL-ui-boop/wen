...
public class FileShareController {
    ...
    @Operation(summary = "分享页面包屑", description = "根据当前目录 id 返回从分享根到该目录的路径，供外链/刷新后还原导航")
    @GetMapping("/{shareId}/path")
    public Result<List<ShareBreadcrumbVO>> getShareBreadcrumb(
            @PathVariable String shareId,
            @RequestParam("parentId") String parentId) {
        return Result.ok(fileShareService.getShareBreadcrumb(shareId, parentId));
    }
    ...
    public ResponseEntity<Resource> downloadShareFile(@PathVariable String shareId, @PathVariable String fileId) {
        try {
            ...
            Long len = fileDownload.getFileSize();
            if (len != null && len > 0) {
                bodyBuilder.contentLength(len);
            }
            return bodyBuilder.body(fileDownload.getResource());
        } catch (Exception e) {
