...
/**
 * 文件流控制器
 * <p>【核心亮点-在线预览】Range 流、策略转换流（如 Office→PDF）与存储 {@link com.xddcodec.fs.storage.plugin.core.IStorageOperationService} 读流对接。</p>
 *
 * @author xddcode
 */
@Slf4j
...
public class FileStreamController {
    ...
    public ResponseEntity<StreamingResponseBody> preview(
        ...
        // 【核心亮点-在线预览】流式输出 + 可 Range 拖动播放/浏览（策略不支持 Range 时走全量）
    ...
    public ResponseEntity<StreamingResponseBody> previewArchiveInner(@PathVariable String tempId) {
        ...
        FileTypeEnum fileType = FileTypeEnum.fromFileInfo(displayName, suffix, displayName);
        PreviewStrategy strategy = strategyManager.getStrategy(fileType);
