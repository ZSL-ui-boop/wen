 ...
 * 文件传输任务服务实现
 * <p>
 * 【核心亮点】与本类直接相关（README「大文件 / 秒传 / 进度 / 完整性」）：
 * <ul>
 *   <li>大文件分片：initUpload → checkUpload 初始化 multipart → uploadChunk → doMergeChunks 合并</li>
 *   <li>断点续传：已传分片写入缓存，doUploadChunk 中跳过重复分片</li>
 *   <li>秒传：CONTENT_MD5 + 工作空间 + 存储配置查重，storage.isFileExist 二次确认后 handleQuickUpload</li>
 *   <li>合并完整性：分片数与各分片 ETag 齐全后才 completeMultipartUpload</li>
 *   <li>实时进度：分片成功后 transferSseService.sendProgressEvent（字节 + 分片序号）</li>
 * </ul>
 *
 * @author xddcode
 */
...
//        }
    ...
    public String initUpload(InitUploadCmd cmd) {
        // 【核心亮点-大文件上传】创建传输任务与缓存，为后续 MD5 校验 / 分片上传做准备
        String userId = StpUtil.getLoginIdAsString();
        String workspaceId = WorkspaceContext.getWorkspaceId();
        String storagePlatformSettingId = StoragePlatformContextHolder.getConfigId();
        try {
    ...
    /**
     * 处理秒传
     * <p>【核心亮点-秒传】新 FileInfo 引用已有 objectKey，避免重复上传字节。</p>
     */
    ...
    /**
     * 上传分片
     * <p>【核心亮点-大文件】异步分片上传，完成后触发自动合并检查。</p>
     *
    ...
    private void doUploadChunk(byte[] fileBytes, UploadChunkCmd cmd) throws IOException {
        ...
        // 【核心亮点-断点续传】已传分片命中缓存则跳过，支持续传与重复请求幂等
        // 检查分片是否已存在（避免重复上传）
    ...
    /**
     * 【核心亮点-大文件 & 完整性】校验分片数与各 Part ETag 后调用存储 completeMultipartUpload，并写入 FileInfo.contentMd5。
     */
