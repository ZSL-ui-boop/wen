...
public class ErrorMessageUtils {
    ...
    public static String extractUserFriendlyMessage(String message) {
        ...
        // 上传「校验」阶段错误常带中文原因在冒号后（如路径不可写），不可截成仅冒号前短语
        if (message.startsWith("文件校验失败:") || message.startsWith("File check failed:")) {
            return message;
        }
        ...
        if (message.matches(".*[\u4e00-\u9fa5]+.*")) {
            ...
            if (colonIndex > 0 && colonIndex < 100) {
                ...
                String afterColon = message.length() > colonIndex + 1
                        ? message.substring(colonIndex + 1).trim()
                        : "";
                // 冒号后仍是中文说明时，保留完整句（避免只剩「文件校验失败」而丢失原因）
                if (afterColon.matches(".*[\u4e00-\u9fa5]+.*")) {
                    return message;
                }
                // 如果冒号前的部分包含中文，只返回这部分（多为「中文摘要: 英文堆栈」）
