...
/**
 * 文件类型枚举（融合预览功能）
 * <p>【核心亮点-在线预览】后缀到类型的映射，供预览策略路由。</p>
 *
 * @Author: xddcode
 * @Date: 2024/12/26
 */
...
public enum FileTypeEnum {
    ...
    /**
     * 结合数据库中的后缀与展示名、原始文件名推断类型。
     * <p>
     * 当展示名不含扩展名（或扩展名不可靠）时，优先使用 {@code suffix} 列，避免 Word 等文档被误判为 {@link #OTHER}。
     */
    public static FileTypeEnum fromFileInfo(String displayName, String suffix, String originalName) {
        FileTypeEnum t = fromSuffix(suffix);
        if (t != OTHER) {
            return t;
        }
        t = fromFileName(displayName);
        if (t != OTHER) {
            return t;
        }
