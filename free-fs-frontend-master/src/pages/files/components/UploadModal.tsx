...
/** 列表预览上限：文件夹成千上万个文件时避免渲染拖垮页面 */
const MAX_PREVIEW_FILES = 150

/** 与 transfer.store 中批量创建任务并发一致思路：限制同时创建的队列任务数 */
...
export default function UploadModal({
  ...
  const handleSubmit = async () => {
    ...
    if (isDirectoryMode) {
      ...
      await pool(BATCH_CREATE_TASK_CONCURRENCY, fileList, (file) =>
          ...
          {fileList.length > 0 && (
            <TooltipProvider>
              <div className='mt-4 max-h-60 space-y-2 overflow-y-auto'>
                {fileList.length > MAX_PREVIEW_FILES && (
                  <p className='text-muted-foreground text-sm'>
                    {t('upload.previewTruncated', {
                      shown: MAX_PREVIEW_FILES,
                      total: fileList.length,
                    })}
                  </p>
                )}
                {fileList.slice(0, MAX_PREVIEW_FILES).map((file, index) => (
                  <FileItem
                    key={index}
                    file={file}
                    index={index}
